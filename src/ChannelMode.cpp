...
/*   Updated: 2024/08/10 16:48:03 by tlassere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	Channel::mode_k_analyse(Client* client_rqst, std::string const& key)
{
	...
	{
		status = FAIL;
		client_rqst->addRPLBuffer("696\n" + client_rqst->getNickName() + " "
			+ this->_name + " k "+ parameter + " :" + error);
		client_rqst->addRPLBuffer("525\n" + client_rqst->getNickName() + " "
