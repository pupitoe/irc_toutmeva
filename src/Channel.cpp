...
void	Channel::RPL_NAMREPLY(Client *client)
{
	...
	buffer = ":" + (std::string)SERVERNAME + " 353 "
...
{
	client->addRPLBuffer(":" + (std::string)SERVERNAME + " 366 "
...
void	Channel::RPL_JOIN_MSG_ERR(Client *client_rqst, std::string const& error,
	char type)
{
	client_rqst->addRPLBuffer(":" + (std::string)SERVERNAME + " "
...
{
	client_rqst->addRPLBuffer(":" + (std::string)SERVERNAME + " 332 "
		+ client_rqst->getNickName() + " " + this->_name + " :"
		+ this->_topic + "\n");
	client_rqst->addRPLBuffer(":" + (std::string)SERVERNAME + " 333 "
		+ client_rqst->getNickName() + " " + this->_name + " "
		+ this->_topic_usr + "\n");
}
	...
	int topicHaveArg)
{
	if (topicHaveArg == false)
	{
		if (this->_topic.empty())
			client_rqst->addRPLBuffer(":" + (std::string)SERVERNAME + " 331 "
	...
	std::string const& userName, std::map<int, Client *>& clientsLst)
{
	...
	if (buffer)
	{
		client_rqst->addRPLBuffer(":" + (std::string)SERVERNAME + " 341 "
			+ client_rqst->getNickName() +
			" " + userName + " " + this->_name + "\n");
	...
	int op)
{
		...
		&& this->_invite_only == false))
	{
		...
		client_rqst->addRPLBuffer(":" + (std::string)SERVERNAME + " 404 "
			+ client_rqst->getNickName()
